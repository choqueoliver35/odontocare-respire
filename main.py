...
import os, random, re, unicodedata, json, hashlib
import psycopg2
...
# Conexión persistente a PostgreSQL (Neon). Se lee de la variable de entorno
# DATABASE_URL; nunca se hardcodea. En desarrollo local se toma del archivo .env.
DATABASE_URL = os.getenv('DATABASE_URL')
if not DATABASE_URL:
    raise RuntimeError('Falta la variable de entorno DATABASE_URL (cadena de conexión PostgreSQL).')
...
class Conn:
    """Envoltura fina sobre psycopg2 que conserva la API usada en todo el código:
    `with conn() as c:`, `c.execute(sql, params).fetchone()/.fetchall()` y `c.commit()`.
    Usa RealDictCursor para que cada fila se comporte como un dict (row['campo'], dict(row)),
    igual que sqlite3.Row. Hace commit al salir del bloque sin excepción y rollback si la hubo.
    """
    def __init__(self):
        self._c = psycopg2.connect(DATABASE_URL, cursor_factory=psycopg2.extras.RealDictCursor)
...
def create_schema(c):
    """Crea el esquema en PostgreSQL solo si no existe (idempotente).
    Reemplaza el FTS5 de SQLite por búsqueda nativa: una columna generada `tsv`
    (tsvector en español) sobre `text`, con índice GIN. Ya no hacen falta triggers.
    """
    c.execute('CREATE TABLE IF NOT EXISTS users(id SERIAL PRIMARY KEY,nombre TEXT,correo TEXT UNIQUE,password_hash TEXT,role TEXT,created_at TEXT)')
    c.execute('CREATE TABLE IF NOT EXISTS threads(id SERIAL PRIMARY KEY,user_id INTEGER,title TEXT,created_at TEXT,updated_at TEXT)')
    c.execute("CREATE TABLE IF NOT EXISTS messages(id SERIAL PRIMARY KEY,thread_id INTEGER,user_id INTEGER,user_message TEXT,bot_response TEXT,stress_level TEXT,stress_score INTEGER,topic_key TEXT,topic_label TEXT,reframe INTEGER DEFAULT 0,out_domain INTEGER DEFAULT 0,model_used TEXT DEFAULT 'interno',created_at TEXT)")
    c.execute('CREATE TABLE IF NOT EXISTS breathing(id SERIAL PRIMARY KEY,user_id INTEGER,thread_id INTEGER,stress_level TEXT,completed INTEGER,created_at TEXT)')
    c.execute('CREATE TABLE IF NOT EXISTS recovery(id SERIAL PRIMARY KEY,user_id INTEGER,thread_id INTEGER,status TEXT,created_at TEXT)')
    c.execute("CREATE TABLE IF NOT EXISTS documents(id SERIAL PRIMARY KEY,filename TEXT,path TEXT UNIQUE,sha256 TEXT UNIQUE,total_pages INTEGER DEFAULT 0,status TEXT DEFAULT 'pendiente',created_at TEXT)")
    ...
    c.execute('CREATE INDEX IF NOT EXISTS doc_chunks_tsv_idx ON doc_chunks USING GIN(tsv)')
    c.execute('CREATE INDEX IF NOT EXISTS doc_chunks_doc_id_idx ON doc_chunks(doc_id)')

def seed_admin_and_demo(c):
    """Siembra/actualiza el administrador y el usuario demo desde las variables de entorno.
    Mantiene una única cuenta de supervisor: si ya existe un admin, actualiza sus credenciales.
    """
    admin=c.execute("SELECT id FROM users WHERE role='admin' ORDER BY id LIMIT 1").fetchone()
    if admin:
        c.execute('UPDATE users SET nombre=%s,correo=%s,password_hash=%s WHERE id=%s',('Administrador',ADMIN_EMAIL,generate_password_hash(ADMIN_PASSWORD),admin['id']))
    else:
        c.execute('INSERT INTO users(nombre,correo,password_hash,role,created_at) VALUES(%s,%s,%s,%s,%s) ON CONFLICT (correo) DO NOTHING',('Administrador',ADMIN_EMAIL,generate_password_hash(ADMIN_PASSWORD),'admin',now()))
    c.execute('INSERT INTO users(nombre,correo,password_hash,role,created_at) VALUES(%s,%s,%s,%s,%s) ON CONFLICT (correo) DO NOTHING',('Estudiante Demo','demo@odontocare.edu.bo',generate_password_hash('demo123'),'estudiante',now()))

def init_db():
    with conn() as c:
        create_schema(c)
        seed_admin_and_demo(c)
        c.commit()
...
def index_document(file_path, display_name=None):
    ...
    with conn() as c:
        existing = c.execute('SELECT id FROM documents WHERE sha256=%s', (file_hash,)).fetchone()
        if existing:
            chunks = c.execute('SELECT COUNT(*) n FROM doc_chunks WHERE doc_id=%s', (existing['id'],)).fetchone()['n']
            return {'ok': True, 'message': 'El documento ya estaba indexado.', 'chunks': chunks}
        row = c.execute('INSERT INTO documents(filename,path,sha256,total_pages,status,created_at) VALUES(%s,%s,%s,%s,%s,%s) ON CONFLICT (sha256) DO NOTHING RETURNING id',
                        (display_name, str(file_path), file_hash, len(pages), 'indexado', now())).fetchone()
        if not row:
            chunks = c.execute('SELECT COUNT(*) n FROM doc_chunks dc JOIN documents d ON d.id=dc.doc_id WHERE d.sha256=%s', (file_hash,)).fetchone()['n']
            return {'ok': True, 'message': 'El documento ya estaba indexado.', 'chunks': chunks}
        doc_id = row['id']
        total_chunks = 0
        for page_num, page_text in pages:
            for idx, chunk in enumerate(make_chunks(page_text)):
                c.execute('INSERT INTO doc_chunks(doc_id,filename,page,chunk_index,text,norm_text,created_at) VALUES(%s,%s,%s,%s,%s,%s,%s)',
...
def search_documents(message, topic_key=None, limit=5):
    ...
    if not terms:
        return []
    # Búsqueda de texto completo nativa de PostgreSQL: websearch_to_tsquery entiende
    # el operador "or" para combinar términos; se ordena por relevancia con ts_rank.
    fts_query = ' or '.join(terms[:16])
    hits = []
    with conn() as c:
        try:
            sql = ("SELECT dc.id, dc.filename, dc.page, dc.text, "
                   "ts_rank(dc.tsv, websearch_to_tsquery('spanish', %s)) AS rank "
                   "FROM doc_chunks dc "
                   "WHERE dc.tsv @@ websearch_to_tsquery('spanish', %s) "
                   "ORDER BY rank DESC LIMIT 35")
            rows = c.execute(sql, (fts_query, fts_query)).fetchall()
        except Exception:
            like = '%' + '%'.join(terms[:3]) + '%'
            rows = c.execute('SELECT id, filename, page, text, 0 AS rank FROM doc_chunks WHERE norm_text LIKE %s LIMIT 35', (like,)).fetchall()
        # Respaldo: si la consulta tsvector no devolvió nada (p. ej. términos sin lema en español),
        # se cae al LIKE sobre norm_text para no quedarnos sin resultados.
        if not rows:
            like = '%' + '%'.join(terms[:3]) + '%'
            rows = c.execute('SELECT id, filename, page, text, 0 AS rank FROM doc_chunks WHERE norm_text LIKE %s LIMIT 35', (like,)).fetchall()
    qset = set(terms)
...
def api_login():
    ...
    with conn() as c:
        u=c.execute('SELECT * FROM users WHERE correo=%s',(email,)).fetchone()
...
def demo_login():
    with conn() as c:
        u=c.execute('SELECT * FROM users WHERE correo=%s',('demo@odontocare.edu.bo',)).fetchone()
        session['user_id']=u['id']; session['user']={'nombre':u['nombre'],'correo':u['correo'],'role':u['role']}
    return jsonify(ok=True)
...
def register():
    ...
    with conn() as c:
        row=c.execute('INSERT INTO users(nombre,correo,password_hash,role,created_at) VALUES(%s,%s,%s,%s,%s) ON CONFLICT (correo) DO NOTHING RETURNING id',(n,e,generate_password_hash(p),'estudiante',now())).fetchone()
        if not row: return jsonify(ok=False,message='Correo ya registrado'),409
        session['user_id']=row['id']; session['user']={'nombre':n,'correo':e,'role':'estudiante'}; c.commit()
    return jsonify(ok=True)
...
def chat():
    ...
    with conn() as c:
        if thread_id:
            row=c.execute('SELECT id FROM threads WHERE id=%s AND user_id=%s',(thread_id,uid)).fetchone()
        else: row=None
        if not row:
            cur=c.execute('INSERT INTO threads(user_id,title,created_at,updated_at) VALUES(%s,%s,%s,%s) RETURNING id',(uid,msg[:45] or 'Nuevo chat',ts,ts)); thread_id=cur.fetchone()['id']
        else:
            c.execute('UPDATE threads SET updated_at=%s WHERE id=%s',(ts,thread_id))
        c.execute('INSERT INTO messages(thread_id,user_id,user_message,bot_response,stress_level,stress_score,topic_key,topic_label,reframe,out_domain,model_used,created_at) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(thread_id,uid,msg,resp,stress['level'],stress['score'],topic_key,TOPICS.get(topic_key,{}).get('label'),1 if 'reencuadre' in stress.get('tags',[]) else 0,1 if out_domain else 0,model_used,ts))
        c.commit()
...
def threads():
    with conn() as c:
        rows=c.execute('SELECT id,title,updated_at FROM threads WHERE user_id=%s ORDER BY updated_at DESC LIMIT 30',(session['user_id'],)).fetchall()
...
def thread(id):
    with conn() as c:
        rows=c.execute('SELECT user_message,bot_response,stress_level,created_at,model_used FROM messages WHERE thread_id=%s AND user_id=%s ORDER BY id',(id,session['user_id'])).fetchall()
...
def breath():
    ...
    with conn() as c:
        c.execute('INSERT INTO breathing(user_id,thread_id,stress_level,completed,created_at) VALUES(%s,%s,%s,%s,%s)',(session['user_id'],d.get('thread_id'),d.get('stress_level'),1,now())); c.commit()
    return jsonify(ok=True)
...
def dashboard():
    ...
    where='WHERE m.created_at>=%s'; params=[since]
    if student!='all': where+=' AND m.user_id=%s'; params.append(student)
    with conn() as c:
        ...
        for lvl in ['Sin alerta','Nivel 1','Nivel 2','Nivel 3']:
            levels.append({'label':lvl,'value':c.execute(f'SELECT COUNT(*) n FROM messages m {where} AND stress_level=%s',params+[lvl]).fetchone()['n']})
        activity=[]
        for i in range(days-1,-1,-1):
            day=(datetime.now()-timedelta(days=i)).strftime('%Y-%m-%d')
            if student!='all':
                n=c.execute('SELECT COUNT(*) n FROM messages WHERE substr(created_at,1,10)=%s AND user_id=%s',(day,student)).fetchone()['n']
            else:
                n=c.execute('SELECT COUNT(*) n FROM messages WHERE substr(created_at,1,10)=%s',(day,)).fetchone()['n']
...
# El script de migración define SKIP_STARTUP_INIT=1 para poder reutilizar las
# funciones de este módulo (create_schema, seed_admin_and_demo, Conn) sin disparar
# el sembrado ni el auto-indexado antes de transferir los datos originales.
